...
SelectBreakouts.prototype = Object.create(Breakouts.prototype,{
	...
	//For selectSort Hybrid
	newArrayfromFinalValues:{
		value:function(){
			this.finalValues();
			this.selectionsFromUser = [];
			for(var i = 0; i < this.breakouts.length; i++){
				var index = this.selectionOrder.indexOf(this.breakouts[i].id);
				if(index >= 0){
					this.selectionsFromUser.push(this.breakouts[i]);
				}
			}
		}
	}
